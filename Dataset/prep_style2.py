...
def _is_cell(path):
    return path[-9:] == '_cell.png'

def _is_hne_cell(path):
    return path[-8:] == '_hne.png'
...
def resize_worker(img_file, sizes):
    ...
    if 'rxrx19' in file or 'CosMx' in file or 'Xenium' in file:
        # only allows resolution 128
        assert len(sizes) == 1 and sizes[0] == 128
        assert 'gene' not in file
        buffer = BytesIO()
        img.save(buffer, format='png')
        out = [buffer.getvalue()]
    else:
...
if __name__ == '__main__':
    ...
    if 'CosMx' in args.path:
        imgset = datasets.ImageFolder(args.path, is_valid_file=_is_cell)
    elif 'Xenium' in args.path:
        imgset = datasets.ImageFolder(args.path, is_valid_file=_is_hne_cell)
    else:
