    ...
    def test_stat(self,
        ...
        for i in ('Lung1', 'Lung2'):
            ...
            df_gene = pq.read_table(str(meta_pth / 'transcripts.parquet'),
                                    filters=[('cell_id', '!=', 'UNASSIGNED'),  # cell_id start with 1
            ...
            df_gene = df_gene.fillna(value=fill_val).astype(type_col)

            print('start summing the total counts')
            # get the total counts
            df_gene['total_counts'] = df_gene.iloc[:, 1:].sum(axis=1)
            df_gene = df_gene.drop(columns=['Unassigned Codeword'])
