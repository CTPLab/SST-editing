...
from style3.models.stylegan2.model import Generator, Discriminator
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, check_save):
    ...
    for idx in pbar:
        ...
        if idx == 0:
            print(fake_img.shape, real_img.shape, real_gene.shape)
        ...
        if get_rank() == 0:
            ...
            if i == 0 or (i + 1) % 5000 == 0:
                with torch.no_grad():
                    g_ema.eval()
                    noise = mixing_gene(real_gene, noise_dim, fix_noise=True)
                    ...
                    sample = torch.cat([sample, sample1, real_img], 0)
                    if sample.shape[1] == 2:
                        sample = torch.cat([-torch.ones_like(sample[:, 0])[:, None], sample], 1) 
                    utils.save_image(
                        sample,
...
if __name__ == "__main__":
    ...
    discriminator = Discriminator(
        args.size, dataset._n_classes, channel_multiplier=args.channel_multiplier, img_chn=args.img_chn
    ).to(device)
    g_ema = Generator(
        args.size, gene_num, args.latent, args.kernel_size, args.n_mlp, channel_multiplier=args.channel_multiplier, img_chn=args.img_chn,
    ).to(device)
    ...
    check_save = check_save.parent / f'{check_save.name}_{args.gene_use}_{args.mixing}_{args.split_scheme}_{args.kernel_size}_{args.latent}'
